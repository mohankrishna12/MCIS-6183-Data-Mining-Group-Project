...
#OUTCOME
- The outcome of our project wasn't successful. from the results we got from all the models, knowing IMDb rating can't help us predict our rating because the result on the plot shows no relationship and errors from the models are large. the same no relationship result when our rating was compared with IMDb rating, year of release, Genres and Directors(with more than four movies).
